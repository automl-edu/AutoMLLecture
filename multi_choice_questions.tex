...
\begin{choices}
    \choice The black-box view is still most commonly used. % Correct
    \choice Grey-box approaches replace algorithm components by learned policies.
    \choice The grey-box view uses more available information than the black-box view. % Correct
...
\begin{choices}
    ...
    \choice propose a set of solution candidates in each iteration of an algorithm, based on previous evaluations. % Correct
    \choice aim to improve the solution quality in each iteration. % Correct
...
\begin{choices}
    \choice an algorithm $\mathcal{A}$ to configure, % Correct
    \choice a configuration space $\Lambda$, % Correct
    \choice a probability distribution $p(\mathcal{D})$ over meta datasets $\mathcal{D}\in\mathbf{D}$, % Correct
    ...
    \choice a cost metric $c$ assessing the cost of a control policy. % Correct
\end{choices}

\question A contextual MDP is ...
\begin{choices}
    \choice much simpler to learn than a standard MDP.
    \choice necessary to learn across datasets. % correct
...
\begin{choices}
    ...
    \choice does potentially make it much easier to learn a successful policy. %Correct
    \choice does potentially make it much harder to learn a successful policy. %Correct
    \choice can be learned via self-paced learning. % Correct
\end{choices}
...
\begin{choices}
    ...
    \choice requires substantial parallel compute resources. % Correct
    \choice only makes use of online information. % Correct
\end{choices}
...
\begin{choices}
    ...
    \choice optimizes the cumulative cost. % Correct
\end{choices}

\question Learning acquisition functions ...
\begin{choices}
    \choice is too expensive to be used in practice.
    \choice replaces the design choice of "Which acquisition function should be chosen", by a learned acquisition function. % Correct
...
\begin{choices}
    \choice worse than standard ML systems due to the double black box view. % Correct
...
\begin{choices}
    ...
    \choice allows to see more detail in the beginning. % COrrect
...
\begin{choices}
    ...
    \choice is possible for $N$-Dimensional spaces through a multi dimensional scaling. % Correct
...
\begin{choices}
    \choice show patterns in the configuration space. % Correct
    \choice are another form of plotting learning curves.
    \choice are hard to interpret due to the number of parameters in the configuration space. % Correct
    \choice can be combined with parameter importance methods to make patterns more apparent. % Correct
\end{choices}

\question Multi-Fidelity Checks allow to ...
\begin{choices}
    \choice measure the distance between budgets.
    \choice gain insights into the usefulness of the chosen fidelities. % Correct
    \choice are another form of "visual analysis of sampling behaviours of AutoML methods".
    \choice see if performances of configurations across the chosen budgets is similar/correlated. % Correct
\end{choices}

\question Select all local hyperparameter importance methods.
\begin{choices}
    \choice Ablation % Correct
    \choice fANOVA
    \choice LPI % Correct
    \choice GP
\end{choices}
...
\begin{choices}
    ...
    \choice $\lambda_4$, $\lambda_3$, $\lambda_1$, $\lambda_2$ % Correct
...
\question Given the table below, compute the local importance around configuration $1, 0, 1, 1$. What is the importance order according to LPI?
    ...
    %\begin{center} % To answer this students only have to look at the loss and don't have to compute exact values. I've given the exact values anyways
      \begin{tabular}{c|cccc}
          & \multicolumn{4}{c}{$\Lambda$} \\
          $L$ & $\lambda_1$ & $\lambda_2$ & $\lambda_3$ & $\lambda_4$ \\
          \hline % total variance = 0.065
          0.4 & $1$ & $0$ & $1$ & $0$ \\ % -> 4 has 0.0225 variance -> ~35% of total variance
          0.5 & $1$ & $0$ & $0$ & $1$ \\ % -> 3 has 0.04 variance -> ~62% of total variance
          0.2 & $0$ & $0$ & $1$ & $1$ \\ % -> 1 has 0.0025 variance -> ~4% of total
          0.1 & $1$ & $0$ & $1$ & $1$ \\
          0.1 & $1$ & $1$ & $1$ & $1$ \\ % -> 2 has no importance
      \end{tabular}
    %\end{center}
\begin{choices}
    \choice $\lambda_3$, $\lambda_2$, $\lambda_4$, $\lambda_1$
    ...
    \choice $\lambda_3$, $\lambda_4$, $\lambda_1$, $\lambda_2$ % Correct
\end{choices}

\question In fANOVA ...
\begin{choices}
    \choice we can easily compute higher-order interaction effects as we can cheaply query the surrogate.
    \choice higher-order interaction effects quickly become too expensive to compute even with the use of a surrogate. % Correct
\end{choices}
