\begin{tikzpicture}[node distance=2.1cm]
    	...
    	minimum height=.75cm] {Algorithm $\algo$};
        		...
        		%	\path (resUL.east |- resBR.north)+(+.9,0.075) node [text=black!75] {control of $\conf$};
        			
        			\end{pgfonlayer}
        			
        			%        \draw[myarrow] (feat.south) -- ($(feat.south |- Agent)+(0,1.125)$);
        			
        			\draw[myarrow] (Agent.north) -- ($(Agent.north)+(0.0,+0.35)$) -- ($(Algo.north)+(0.0,+0.35)$) node [above,pos=0.5] { apply action $a^{(t)}$} node [below,pos=0.5] {set $\conf$} -- (Algo.north);
