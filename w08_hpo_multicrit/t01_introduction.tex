...
A \textit{simple} approach would be to formulate all but one objective function simplified as a secondary condition. %FIXME: JR: unclear to me
...
Given a multi-criteria optimization problem
    $$\min_{\conf \in \pcs} \cost(\conf) = \left(\cost_1(\conf), ..., \cost_m(\conf)\right), \quad \cost_i: \pcs \to \realnum.$$ %FIXME: JR: Looks not entirely right. min on right too?
...
$$
%FIXME: JR: Again, min also on the right?
...
$$
%FIXME: JR: Again, the notation. Also now `:=` ?
