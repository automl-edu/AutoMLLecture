\begin{tikzpicture}[node distance=2.1cm]
        			...
        			\node (Agent) [activity,
    	minimum height=.75cm] {Controller $\policy$};
        			
        			\node (Algo) [activity, right of=Agent, xshift=3cm,
    	minimum height=.75cm] {Algorithm $A$};
        			...
        			\path (resUL.east |- resBR.north)+(+.9,0.075) node [text=black!75] {control of $\param_i$};
        			
        			\end{pgfonlayer}
        			
        			%        \draw[myarrow] (feat.south) -- ($(feat.south |- Agent)+(0,1.125)$);
        			
        			\draw[myarrow] (Agent.north) -- ($(Agent.north)+(0.0,+0.35)$) -- ($(Algo.north)+(0.0,+0.35)$) node [above,pos=0.5] { apply action $a_t$} node [below,pos=0.5] {set $\param_i=v$} -- (Algo.north);
        			...
        			\path (resBR.east |- resUL.center)+(.8, -0.175) node [text=black!32.5] {$\mdata$};
