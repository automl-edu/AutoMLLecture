...
\begin{document}
	\gccs
	Having learned about different ways to empirically evaluate the performances of algorithms and automl systems in this exercise you will now implement some of these techniques. Add your code creating plots and outputting statistics to \texttt{main.py} (callable as \verb|python main.py|). Furthermore, combine all plots and answers to the questions into one PDF. We expect all plots to have axes labels and a legend!
	\begin{questions}
		\titledquestion{Visualization and Evaluation}[3]
		\verb|data.csv| contains \textit{error} values of two algorithms $A$ and $B$ on $n=419$ datasets. The method \verb|load_data()| loads this dataset as an $n \times 2$ \textit{numpy array}.
		\begin{parts}
			...
			\begin{itemize}
				\item algorithm $A$ achieved an \textit{error} value that is $0.1$ lower than that of $B$
				\item algorithm $B$ achieved an \textit{error} value that is $0.1$ lower than that of $A$
				\item other
			\end{itemize}
			...
			\part[0.5] Plot the empirical cumulative distribution function (eCDF) of $A$ and $B$ as shown in the lecture. What are the probabilities of $A$ and $B$ to achieve an \textit{error} value of $0.4$ and lower (it is okay to estimate the value from the plot).
			...
			\part[1] Implement a paired permutation test to determine whether the mean of $A$ is better than the mean of $B$. In your solution state what is $H_0$, $H_1$ and which $\alpha$-value you used?
		\end{parts}
