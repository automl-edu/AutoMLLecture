...
\begin{document}
    ...
    \item The test set should only be touched once, so we have no way of
        ``cheating''. The test dataset is only used once \emph{after} a model is completely trained, after deciding for example on specific hyper-parameters. 
