...
%	\myblock{Definition: meta-learning for HPO}{
		Given:
	    \myit{
	        \item a set of prior tasks: $t_{j} \in \mathcal{T}$,
	        \item a set of new tasks: $t_{\text{new}} \in \mathcal{T}$,
\pause
	        \item a set of learning algorithms, fully defined by $\theta_{i} \in \Theta$
\pause
	        \item a set of prior evaluations: $\dataset$, where $\dataset_{i, j} = \cost(\theta_{i}, t_{j})$
	        \item a (possibly empty) set of known evaluations $\cost_{i, \text{new}}$ on a new task $t_{\text{new}}$
		}
		\bigskip
\pause
...
\vspace*{-0.3cm}
    ...
    \begin{itemize}
        ...
        \item \emph{Method:} mostly \alert{greedy} on $\dataset_{\text{meta}}$
