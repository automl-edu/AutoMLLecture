...
%	\end{itemize}
    ...
    \myit{
		...
		\item Acquisition function now also needs to select $z$: take into account cost of evaluations
	}
\end{itemize}
...
\pause
		...
		\begin{enumerate}
			...
			\item Select $\conf$ that maximizes the following acquisition function:
		\end{enumerate}
\end{itemize}
\vspace*{0.5cm}
...
%----------------------------------------------------------------------
%----------------------------------------------------------------------
\begin{frame}[c]{Fast Bayesian Optimization on Large Datasets (Fabolas) \litw{\href{http://proceedings.mlr.press/v54/klein17a/klein17a.pdf}{Klein et al. 2017}}}

\begin{itemize}
		\item Entropy search for multi-fidelity optimization 
		\pause
\medskip
...
\bigskip
\myit{
	\item However, Faboloas is limited to low-dimensional, continuous functions
	\item Standard GP model causes large \alert{overhead} -- cannot exploit too many cheap evaluations
	\myit{
		\item BOHB is much simpler and more flexible
	}
}
