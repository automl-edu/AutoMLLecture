...
%				$\cutoff_i \xleftarrow{}$ AdaptCutoff$(\cutoff_{max}, \langle(\conf_j, \cost_j)\rangle_{\conf_j = \conf^+})$;\\
				...
				\pause
				\If{average cost of $\confI{\bocount} >$ average cost of $\hat{\conf}$ across $\insts^{(\bocount)}$ and $\mathbf{s}^{(\bocount)}$}{break;}
				\pause
				\ElseIf{average cost of $\confI{\bocount} <$ average cost of $\hat{\conf}$ and $\insts^+ =  \insts^{(\bocount)}$  and $\mathbf{s}^{(\bocount)} = \mathbf{s}^+$}{
