...
\While{$b$ remains} {
	...
	\pause
	$\hat{\conf}, \hist$  $\leftarrow$ intensify($\confs_{challengers}$, $\hat{\conf}$); \tcp*{racing and capping}
}
\pause
\Return{$\hat{\conf}$}
\caption{SMAC}
...
\begin{algorithm}[H]
	\scriptsize
	\Input{%
		 candidate configurations $\confs_{new}$, cutoff $\cutoff_{max}$, previously evaluated runs $\hist$, budget $\bobudget$, incumbent $\hat{\conf}$}
		\While{$\confs_{new}$ not empty}{
			...
			$c$ $\xleftarrow{}$ EvaluateRun$(\hat{\conf}, \inst, s, \cutoff_{max})$;\\
			$\hist \xleftarrow{} \hist \cup (\hat{\conf}, \inst, s, \cost)$;\\
			\pause
			\While{true}{
				 $\insts^+, \mathbf{s}^+ \xleftarrow{} $getAlreadyEvaluatedOn$(\hat{\conf}, \hist)$;\\
...
%				$\cutoff_i \xleftarrow{}$ AdaptCutoff$(\cutoff_{max}, \langle(\conf_j, \cost_j)\rangle_{\conf_j = \conf^+})$;\\
				...
				\pause
				\If{average cost of $\confI{\bocount} >$ average cost of $\hat{\conf}$ across $\insts_+ \setminus \insts^{(\bocount)}$ and $\mathbf{s}^+ \setminus \mathbf{s^{(\bocount)}}$}{break;}
				\pause
				\ElseIf{average cost of $\confI{\bocount} <$ average cost of $\hat{\conf}$ and $\insts_+ =  \insts^{(\bocount)}$  and $|\mathbf{s}^{(\bocount)}| = |\mathbf{s}^+|$}{
					$\hat{\conf} \xleftarrow{} \confI{\bocount}$;
					}
			}
		\pause
		\If{time spent exceeds $\bobudget$ or $\confs_{new}$ is empty} {
			\Return{$\hat{\conf}, \hist$}
		}
		}
\end{algorithm}
\end{frame}
