...
\begin{algorithm}[H]
	...
	\Input{%
		...
		\While{$\confs_{new}$ not empty}{
			...
			$c$ $\xleftarrow{}$ EvaluateRun$(\incumbent, \inst, s, \cutoff_{max})$;\\
			$\hist \xleftarrow{} \hist \cup (\incumbent, \inst, s, \cost)$;\\
			\pause
			\While{true}{
...
%				$\cutoff_i \xleftarrow{}$ AdaptCutoff$(\cutoff_{max}, \langle(\conf_j, \cost_j)\rangle_{\conf_j = \conf^+})$;\\
				$c_i \xleftarrow{}$ EvaluateRun$(\confI{\bocount}, \instI{\bocount},  s^{(\bocount)}, \cutoff^{(\bocount)})$;\\
