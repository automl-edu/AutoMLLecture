...
\begin{footnotesize}
	...
	\While{$b$ remains} {
		...
		\pause
		get first element $(\inst^{(k)}, \kappa)$ from $Q_{\hat{\conf}}$;\\
		\pause
		Run $\hat{\conf}$ on $\inst^{(k)}$ capped at $\kappa$;\\
		\If{terminates}{$R(\hat{\conf}, \inst^{(k)})$ := t;}
		\pause
		\Else{
		$R(\hat{\conf}, \inst^{(k)}) := \kappa$;\\
		Insert $(\inst^{(k)}, 2\cdot \kappa)$ at the end of $Q_{\hat{\conf}}$;
		}
		\pause
		Replenish queue $Q_{\hat{\conf}}$ if too small;	
	}
	\pause
	\Return{$\hat{\conf}:= \argmin_{\conf \in \confs} \sum_{k=1}^N R(\lambda, \inst^{(k)})$}
