...
%----------------------------------------------------------------------
\myframe{One-shot models: convolutional neural fabrics \litw{\href{https://arxiv.org/pdf/1606.02492.pdf}{Saxena and Verbeek, 2017}}}{
	\centering
	\includegraphics[width=0.7\textwidth]{images/conv_fabric_1.png}\\
	
\bigskip
	\myit{
		\item A \alert{convolutional neural fabric} / \alert{lattice} / \alert{one-shot model} 
		is a big model that comprises an exponential number of architectures
	\medskip
		\myit{
...
\visible<2->{
		...
		\myit{
			\item Weights need to work well on average across all architectures
			\item They are not optimized for the top-performing architecture
		}
}
	}
\bigskip
...
\bigskip	
	\centering
	\visible<1->{\includegraphics[width=0.2\textwidth]{images/one_shot_model_1.png}\quad\quad\quad\quad\quad\quad}
	\visible<2->{\includegraphics[width=0.2\textwidth]{images/drop_path_1.png}\quad\quad}
...
\myframetop{How to utilize the trained one-shot model?}{
    ...
    \begin{itemize}
		...
		\myit{
\pause
			\item[$1$.] Sample uniformly at random $M$ architectures and rank them based on their validation error \alert{using the one-shot model parameters} (retraining from scratch is computationaly expensive)
			%\item[-] Select the top $K$, where $K < M$, and evaluate those on the full validation set, again using the one-shot parameters.
\pause
