...
In the first step of an iteration, $\lambda$ parents are chosen, who create offspring in the next step. %FIXME: JR: lambda is HP configuration! double notation
...
Tuning \emph{xgboost} with random search, grid search and simulated annealing using a 5CV on the \texttt{spam} and \texttt{sonar} data set for AUC:
\begin{columns}
...
\begin{column}{0.7\textwidth}
  \begin{center}
  \begin{figure}
  \only<1>{\includegraphics[width=\textwidth]{images/benchmark_curve.png}}
  \only<2>{\includegraphics[width=\textwidth]{images/benchmark_scatter.png}}
  \end{figure}
  \end{center}
\end{column}
\end{columns}
