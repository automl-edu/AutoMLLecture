...
\begin{itemize}
	...
	\item Only use mean and stdevarcSepA=[0pt]
...
\begin{itemize}
	\item Remark: x-axis should be on log-scale
    \pause
    \smallskip
...
%----------------------------------------------------------------------
%----------------------------------------------------------------------
\begin{frame}[c]{Uncertainties in Ranking Plots}

Given:
\begin{itemize}
	\item $k$ systems you want compare
	\item $n$ runs of each system
	\item $d$ datasets
\end{itemize}

\pause
\begin{block}{Across Datasets ($d > 1$)}

\begin{enumerate}
	\item for each system and dataset, average performance across the $n$ runs
	\item compute ranking on each datasets
	\item compute mean and stdev across rankings
\end{enumerate}

\end{block}

\pause
\begin{block}{On a single dataset ($d = 1$)}
	\begin{enumerate}
		\item for each system, sample a single run out of the $n$ runs
		\item compute ranking on these samples
		\item repeat 1. and 2. at least $10\ 000$ times to obtain many rankings
		\item compute mean and stdev across rankings
	\end{enumerate}
...
%-----------------------------------------------------------------------
%----------------------------------------------------------------------
\begin{frame}[c]{Paired Permutation Test}

Given: 

\begin{itemize}
	\item \alert{ordered} observations $[x_1, ... x_n]$ in $X$ (resp. in $Y$) where each observation is attributed to concrete objects
\end{itemize}
...
\begin{itemize}
	...
	\begin{enumerate}
		\item $X'$ = [] and $Y'$ = []
		\item for each \alert{pair of observations $\langle x_i, y_i \rangle$} sample whether (i) you put $x_i$ into $X'$ and $y_i$ into $Y'$ or (ii) $x_i$ into $Y'$ and $y_i$ into $X'$. 
		\item add test statistic based on $X'$ and $Y'$ to $S$
	\end{enumerate}
\end{itemize}
