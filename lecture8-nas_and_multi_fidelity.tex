...
\pause
	\myit{
		\item Relies on manually-created parametric curves
...
\myit{
	\item Use a Gaussian process with input $\vtheta$ and $t$; special kernel for $t$
...
\myit{
	...
	\pause
	\myit{
		\item \goleft[.45]{Relies on manually-created parametric curves}
		\item \goleft[.45]{Cannot quickly integrate new information from extending the curve / new runs}
	}
}
\end{frame}
%-----------------------------------------------------------------------
...
\medskip		
		...
		\myit{
			\item Small cell search space with 423k unique architectures
